...
- `isLoading`: no cache and in flight → Return `<LoadingView />`
...
Help screens should have a section for "Actions" if there are buttons within the header, either within a menu or directly. The back button should not be included unless it has very special properties (such as `SiteUIScreenBase.tsx`). Actions should be listed in order of header then within any menus. There does not need to be a help entry for the actions menu itself. If there is a button for Help, it should be last. Any privileged actions should go in a "Privileged Actions" TitleView after "Action".
...
All help screens should have `isStack={true}` and `overScroll={true}` on the `ScrollingContentView`.

`HelpTopicView`s that are contained within a `HelpChapterTitleView` should be children of the `HelpChapterTitleView`.

If a screen contains swipeable components, document that along with the swipe specific actions in a dedicated `HelpChapterTitleView`. Order them by starting on the default right swipable pane, going left to right. Followed by the default left pane, also left to right. Likewise add the `SelectionHelpTopicView` if items in a screen implement selection.
...
Use the helpers in `src/Libraries/Platform/Detection.ts` for any platform detection that may be necessary.

## Menus

We call three-dots menus Actions Menus. The visibility should be determined from `useMenu.ts`. The anchor should be `MenuAnchor.tsx`. If passing a `props.onClose?` callback to a mutating menu item that callback must be performed in the `onSettled` block of the mutation otherwise it could get unmounted before executing.

"Toggle" and "Navigation" style menu items such as `NeedsPhotographerMenuItem` and `HelpMenuItem` should not provide the `onClose`. The latter because we hook into the global navigation system to close menus on navigate. "Action" style menu items such as `ShareMenuItem` must provide it.

## Queries

### Mutations

Mutations will generally need to invalidate certain queries in the cache. For example:
...
Avoid hardcoding query keys in these functions and instead rely on a `getCacheKeys()` function on the type of data. If one does not exist, make one.
