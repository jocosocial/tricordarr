...
export const OpenInSchedMenuItem = (props: OpenInSchedMenuItemProps) => {
  ...
  const schedUrl = `${appConfig.schedBaseUrl}/event/${props.eventUid}`;

  const handlePress = useCallback(() => {
    props.closeMenu();
    Linking.openURL(schedUrl).catch(error => {
  ...
  }, [props, schedUrl, setSnackbarPayload]);
  ...
  return (
    <Menu.Item
      title={'Open in Sched'}
      leadingIcon={AppIcons.schedOpen}
      onPress={handlePress}
