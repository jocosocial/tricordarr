...
const ImageItem = ({
  ...
  const maxZoomScale = Math.max(
    MIN_SCREEN_ZOOM,
    imageDimensions ? (imageDimensions.width / screenSizeDelayedForJSThreadOnly.width) * MAX_ORIGINAL_IMAGE_ZOOM : 1,
  );
            ...
            <FastImage
              ...
              onLoad={
                hasLoaded
                  ...
                  : e => {
                      setHasLoaded(true);
                      onLoad({width: e.nativeEvent.width, height: e.nativeEvent.height});
                    }
              }
            />
          </Animated.View>
