...
import {Footer} from '#src/Components/Lightbox/chrome/Footer';
import {Header} from '#src/Components/Lightbox/chrome/Header';
import ImageItem from '#src/Components/Lightbox/pager/ImageItem/ImageItem';
import {type Lightbox} from '#src/Components/Lightbox/state';
import {
  type Dimensions,
  type ImageSource,
  type LightboxImage,
...
const systemBarsStyle = {statusBar: 'light' as const, navigationBar: 'light' as const};
...
function canAnimate(lightbox: Lightbox, reduceMotion: boolean): boolean {
  if (reduceMotion) {
    return false;
  }
...
export default function ImageViewRoot({
  ...
  onRequestClose: () => void;
  onPressSave: (image: LightboxImage) => void;
  onPressShare: (image: LightboxImage) => void;
  viewerMessage: StringOrError;
  setViewerMessage: (message: StringOrError) => void;
}) {
  'use no memo';
  ...
  }, [nextLightbox, openProgress, thumbRects, reduceMotion]);
...
function ImageView({
  ...
  const {images, allowSave = true, allowShare = true} = lightbox;
  ...
  return (
    <Animated.View style={[styles.container, containerStyle]}>
      ...
      <View style={styles.controls} pointerEvents={'box-none'}>
        <Animated.View style={animatedHeaderStyle} pointerEvents={'box-none'} renderToHardwareTextureAndroid>
          <Header
            onRequestClose={handleRequestClose}
            onPressShare={allowShare ? () => onPressShare(images[imageIndex]) : undefined}
            onPressSave={allowSave ? () => onPressSave(images[imageIndex]) : undefined}
            onPressInfo={() => setIsInfoVisible(visible => !visible)}
        ...
        <Animated.View
          ...
          renderToHardwareTextureAndroid={!isInfoVisible}>
          <Footer
            fileName={images[imageIndex].metadata.fileName}
            image={isInfoVisible ? images[imageIndex].metadata : undefined}
            onLayout={event => setFooterHeight(event.nativeEvent.layout.height)}
          />
          <ImageViewerSnackbar message={viewerMessage} setMessage={setViewerMessage} bottomOffset={footerHeight} />
        </Animated.View>
      </View>
    </Animated.View>
  );
...
function LightboxImage({
  ...
  const {width: widthDelayedForJSThreadOnly, height: heightDelayedForJSThreadOnly} = useWindowDimensions();
  ...
  const transforms = useDerivedValue<LightboxTransforms>(() => {
    ...
    if (isActive && imageAspect && openProgressValue < 1) {
      ...
      if (thumbRect) {
        return interpolateTransform(openProgressValue, thumbRect, safeArea, imageAspect, thumbBorderRadius);
      }
    }
    return {
      isHidden: false,
...
function interpolateTransform(
  ...
  const borderRadius = interpolate(progress, [0, 1], [sourceBorderRadius / (initialScale * initialCropScaleX), 0]);
...
function withClampedSpring<T extends AnimatableValue>(value: T, config: WithSpringConfig): T {
  'worklet';
