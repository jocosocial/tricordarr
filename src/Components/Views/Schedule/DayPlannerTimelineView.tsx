...
export const DayPlannerTimelineView = forwardRef<ScrollView, DayPlannerTimelineViewProps>(({items, dayStart, dayEnd}, ref) => {
  ...
  const getGridLineStyle = (slotType: TimeSlotType) => {
    switch (slotType) {
      case 'hour':
  ...
  const styles = StyleSheet.create({
    ...
    timeLabel: {
      ...
      marginTop: 0, // Align label with grid line
    },
    eventsColumn: {
      flex: 1,
      position: 'relative',
    ...
    return (
      <View style={styles.emptyMessage}>
        <Text style={styles.emptyText}>
          No items in your Day Planner for this day.
        </Text>
        <Text style={styles.emptyText}>
    ...
    <ScrollView ref={ref} style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={[styles.timeColumn, {height: timelineHeight}]}>
        {timeSlots.map((slot, index) => (
          <View
            key={index}
            style={[styles.timeSlot, getGridLineStyle(slot.slotType)]}>
      ...
      <View style={[styles.eventsColumn, {height: timelineHeight}]}>
        ...
        <View style={styles.gridLines}>
          {timeSlots.map((slot, index) => (
            <View
              key={index}
              style={[styles.gridLine, getGridLineStyle(slot.slotType)]}
            />
          ))}
        </View>
