...
type Props = StackScreenProps<ForumStackParamList, ForumStackComponents.forumRecentScreen>;

export const ForumThreadRecentScreen = (props: Props) => {
  return (
    <LoggedInScreen>
      <PreRegistrationScreen helpScreen={CommonStackComponents.forumCategoryHelpScreen}>
        <DisabledFeatureScreen feature={SwiftarrFeature.forums} urlPath={'/forum/recent'}>
          <SelectionProvider>
            <ForumThreadRecentScreenInner {...props} />
          </SelectionProvider>
...
const ForumThreadRecentScreenInner = ({navigation}: Props) => {
  ...
  useEffect(() => {
    navigation.setOptions({
      ...
      title: enableSelection ? `Selected: ${selectedItems.length}` : 'Recently Viewed',
    });
  }, [getNavButtons, navigation, enableSelection, selectedItems.length]);

  return (
    <AppView>
      <ForumThreadsRelationsView relationType={ForumRelationQueryType.recent} onDataChange={setForumListData} />
    </AppView>
  );
};
